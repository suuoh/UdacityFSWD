...
function populateInfoWindow(marker, infowindow) {
    if (infowindow.marker != marker) {
        ...
        if (place.wikipedia) {
            // Check if Wikipedia library call resulted in error
            if (place.wikipedia !== "Error")
                infoContent += "<p><a href='" +  place.wikipedia + "' target='_blank'>Wikipedia</a></p>";
            else
                infoContent += "<p><strong>Error:</strong> Wikipedia API could not be loaded.</p>";
        }
...
function getWikiLinks() {
    ...
    PLACES.forEach(function(place) {
        if (place.category !== "Restaurant") {
            ...
            .done(function(data) {
                place.wikipedia =  "https://en.wikipedia.org/wiki/" + data[1][0];
            })
            .fail(function() {
                place.wikipedia = "Error";
                console.log("Oops! There was an issue loading the Wikipedia library. Please try again by refreshing the page.");
            });
        }
    });
}

// Log and show message if Google Maps library fails
function gmapsError() {
    console.log("Oops! There was an issue loading the Google Maps library. Please try again by refreshing the page.");
    document.getElementById("map").innerHTML = "<h4>Error: Google Maps API could not be loaded.</h4>";
}
